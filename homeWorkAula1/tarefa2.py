#IDENTIFICAR VARIAVEIS


'''Nomear variavel cidade de residencia'''
cidade_resd = "Fundao"

'''Nomear variavel viagem dos sonhos
A Russia da literatura do sec XIX
Tolstoi, Dostoievsvy, Gogol,  Pushkin
Noites Brancas, Guerra e Paz, Anna Kareninna, Crime e Castigo
São Petesburgo, Moscovo'''
viagem_dest = "Russia"


#ESCREVER PROGRAMA

''''''

print ("Atualmente resido em", cidade_resd,".""É uma cidade no interior de Portugal, mas não nasci aqui. Contudo, sei que tenho um grande sonho: de fazer uma viagem e conhecer a", viagem_dest,".")

print ("Tenho a certeza que ficaria encantada com esse país")
      

